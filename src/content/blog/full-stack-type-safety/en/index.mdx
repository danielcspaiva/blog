...
### Building with tRPC and Typescript ORMs
...
React and Next.js introduced [Server Components](https://nextjs.org/docs/app/building-your-application/rendering/server-components) and [Server Actions](https://nextjs.org/docs/app/building-your-application/data-fetching/server-actions-and-mutations), allowing you to execute server-side code within React components. This approach offers a new way to achieve type safety, combining the best of both worlds: TypeScript's type safety with server performance.

With Server Components, you can directly access your database and get automatic types from Prisma, eliminating the need for fetch code in the frontend. This results in better initial performance and less boilerplate code. Here's a practical example:
...
Performance also requires special attention. Server Components can block rendering if the operation is slow, requiring strategies like streaming, Suspense boundaries, and data caching. Additionally, the separation between Server and Client Components can be confusing initially, and the technology is still relatively new, with documentation and best practices evolving.

Server Components are ideal for static or semi-static content, applications with many database operations, and cases where SEO is critical. Server Actions shine in forms, operations requiring server-side validation, and situations where you want to avoid traditional REST APIs.

For applications that need more interactivity, a hybrid approach might be more suitable, combining Server Components with other solutions like tRPC. This combination allows you to maintain type safety while offering more flexibility for interactivity.
