...
description: Como é a experiência Full Stack Type Safety e por que não consigo voltar atrás
date: 2025-04-14
...
Full stack type safety conecta seu banco de dados, backend e frontend em um sistema de tipos unificado. Usando ferramentas como [Prisma](https://www.prisma.io/) ou [Drizzle](https://orm.drizzle.team/) para operações de banco de dados e [tRPC](https://trpc.io/) para comunicação de API, você pode:
...
- Ter segurança de tipos de ponta a ponta em toda a sua stack
- Reduzir programação defensiva no seu código
- Refatorar com segurança e confiança

O resultado? Código mais limpo, mais fácil de manter, com menos bugs e melhor DX para o seu time.
...
Considere este exemplo comum que ilustra o problema:
...
### Definições de Tipos Manuais: Uma Solução Incompleta
...
### Inferência de Tipos vs Anotação Manual
...
A verdadeira segurança de tipos de ponta a ponta conecta seu banco de dados, backend e frontend em um sistema de tipos unificado onde os tipos fluem automaticamente através de cada camada.
...
- Inferindo automaticamente tipos do seu código backend
...
No lado esquerdo temos o código do servidor, e no lado direito temos o código do cliente. Assim que você faz uma alteração no código do servidor, o tRPC fornece feedback imediato no lado do cliente. Se houver uma incompatibilidade de tipos, você verá erros de tipo no cliente, informando exatamente o que precisa ser atualizado.

Quando você altera o tipo de retorno do servidor, o tRPC infere automaticamente o novo tipo e o fornece ao cliente. Isso significa que sua IDE mostrará exatamente quais propriedades estão disponíveis e quais tipos elas têm, tudo em tempo real.

### Construindo com tRPC e ORMs de Banco de Dados
...
O fluxo de tipos cria uma cadeia contínua:
1. O esquema do banco de dados define a estrutura
2. O Prisma gera tipos TypeScript
3. O tRPC infere tipos de retorno do Prisma
...
1. O Prisma atualiza os tipos TypeScript após a regeneração
2. O TypeScript sinaliza imediatamente o código backend que precisa de atualizações
3. O frontend também mostra onde você precisa se adaptar à mudança de esquema
...
- Validação em tempo de execução antes que os dados cheguem à lógica de negócios
- APIs auto-documentadas com requisitos de entrada explícitos
...
Enquanto [tRPC](https://trpc.io/) com [Prisma](https://www.prisma.io/) ou [Drizzle](https://orm.drizzle.team/) oferecem soluções elegantes, existem outras alternativas de obter resultados parecidos:

### Server Components e Server Actions

O React e Next.js introduziram [Server Components](https://nextjs.org/docs/app/building-your-application/rendering/server-components) e [Server Actions](https://nextjs.org/docs/app/building-your-application/data-fetching/server-actions-and-mutations), permitindo executar código no servidor dentro de componentes React. Esta abordagem oferece uma nova maneira de alcançar type safety, combinando o melhor dos dois mundos: a segurança de tipos do TypeScript com a performance do servidor.

Com Server Components, você pode acessar diretamente o banco de dados e obter tipos automáticos do Prisma, eliminando a necessidade de código de fetch no frontend. Isso resulta em melhor performance inicial e menos código boilerplate. Veja um exemplo prático:
...
Para mutações, Server Actions combinam perfeitamente com Zod para validação em tempo de execução:
...
A performance também requer atenção especial. Server Components podem bloquear a renderização se a operação for lenta, exigindo estratégias como streaming, Suspense boundaries e cache de dados. Além disso, a separação entre Server and Client Components pode ser confusa inicialmente, e a tecnologia ainda é relativamente nova, com documentação e melhores práticas em evolução.

Server Components são ideais para conteúdo estático ou semi-estático, aplicações com muitas operações de banco de dados e casos onde SEO é crítico. Já Server Actions brilham em formulários, operações que requerem validação no servidor e situações onde você quer evitar APIs REST tradicionais.

Para aplicações que precisam de mais interatividade, uma abordagem híbrida pode ser mais adequada, combinando Server Components com outras soluções como tRPC. Esta combinação permite manter a type safety enquanto oferece mais flexibilidade para interatividade.
...
A segurança de tipos de ponta a ponta transforma o processo de desenvolvimento fornecendo:
...
[Theo](https://www.youtube.com/@t3dotgg) - Por seu conteúdo excepcional sobre tRPC e segurança de tipos, que me ajudou a entender a importância e o poder da segurança de tipos de ponta a ponta no desenvolvimento web moderno.
...
[Colin McDonnell](https://github.com/colinhacks) - Por criar o Zod, a poderosa biblioteca de validação de esquemas com foco em TypeScript que se tornou uma ferramenta essencial para o desenvolvimento seguro em termos de tipos.

[Tanner Linsley](https://twitter.com/tannerlinsley) - Por criar o TanStack Query (anteriormente React Query), que revolucionou o fetching de dados em aplicações React e fornece a base para a experiência do lado do cliente do tRPC.

[TkDodo](https://twitter.com/tkdodo) - Por suas valiosas contribuições para o ecossistema TanStack Query e seu conteúdo educacional excepcional que ajudou inúmeros desenvolvedores a dominar o fetching de dados e o gerenciamento de estado.

[Julius](https://github.com/juliusmarminge) - Por suas contribuições significativas ao tRPC, incluindo o desenvolvimento do adaptador tRPC para Next.js e seu trabalho na melhoria da experiência do desenvolvedor no ecossistema tRPC.
