...
export function getLocalizedProject(
  ...
  const localizedProject = projects.find(
    (project) => getSlugFromId(project.id) === slug && project.data.locale === locale,
  );
  ...
  return projects.find((project) => getSlugFromId(project.id) === slug && !project.data.locale);
}
